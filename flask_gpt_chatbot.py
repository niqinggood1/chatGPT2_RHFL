...
if args.ppo_train:
    args.topk = 1
    gpt2_model,gpt2_model_ref,writer= get_ppo_basemodel(  config, args )
    model = gpt2_model.transformer
    ppo_trainer         = PPOTrainer(gpt2_model, gpt2_model_ref, tokenizer, **config)


from os.path import join, exists
global epoch,feedbackcnt
epoch=0;feedbackcnt=0;
...
def feedback():
    ...
    idx  = batch['resp_idx'][text]
    batch['rewards'][idx] = 0.2  if feedback=='good' else -0.2
    print('feedback  the query of response:', global_key_value.setdefault( text,'not found !!!') )
    if feedbackcnt>=config['batch_size'] and args.ppo_train and len(batch)>= config['batch_size']:
        print( '启动训练','len batch',len(batch) )
        query_tensors          =   [ torch.tensor(t).long().to(device) for t in batch["tokens"] ]
        response_tensors       =   [ torch.tensor(t).long().to(device) for t in batch["response_tensors"] ]#  # 直接采用历史数据 [gpt2_tokenizer.decode(r.squeeze()) for r in response_tensors]
        rewards                =   torch.tensor(batch['rewards']).to(device)  #rewards = torch.tensor(rewards).to(device)
        print('feedback rewards',rewards )

        ppo_trainer=train_ppo_network( ppo_trainer,tokenizer, batch, query_tensors, response_tensors, rewards, device, writer,epoch,args.max_len )
        batch = init_batch()
        ############## save model
        ...
        model_to_save = ppo_trainer.model.transformer.module if hasattr( ppo_trainer.model.transformer, 'module') else  ppo_trainer.model.transformer
...
def gen_response():
    ...
    input_ids = input_ids[-500:]  ; print('history sentence: ',tokenizer.decode( input_ids ) )
    ...
    batch.setdefault('resp_idx', {})
    batch['resp_idx'].setdefault(resp_text , len(batch['resp_idx'])  )

    #batch['response'] = [gpt2_tokenizer.decode(r.squeeze()) for r in response_tensors ]
    return  json.dumps({'message': 'Ok', 'success': 1,'content': resp_text  })
