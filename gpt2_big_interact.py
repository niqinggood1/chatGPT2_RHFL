...
def main():
    ...
    print('text first decode:',tokenizer.decode(text_ids)    )
    ...
    while True:
        try:
            ...
            #print('input_ids2',input_ids,'decode:',tokenizer.decode(response))
            ...
            for _ in range(args.max_len):
                ...
                next_token_logits = logits[0, -1, :] #outputs[0][:, -1, :]
                ...
                if next_token == tokenizer.sep_token_id or  next_token ==tokenizer.eos_token:  # 遇到[SEP]则表明response生成结束
                    break
                response.append(next_token.item())
                input_ids = torch.cat((input_ids, next_token.unsqueeze(0)), dim=1)
            #response = model.generate(  input_ids,max_new_tokens= args.max_len,num_beams=args.topk ).squeeze()[-args.max_len:]
